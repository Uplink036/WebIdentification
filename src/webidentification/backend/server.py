...
assert (
    len(_get_all_models()) > 0
), f"No model files found in {MODEL_DIR}. Please ensure at least one .pt file is present."
...
def _load_model_from_path(model_path: Path):
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to load model {model_path.name}: {e}"
        )
...
def _build_model(model_name: str | None = None):
    ...
    if model_name:
        if model_name not in names:
            raise HTTPException(
                status_code=400,
                detail=f"Model {model_name} not found. Available models: {names}",
            )
...
async def predict(request: Request, model: str | None = None):
    ...
    try:
        ...
        logger.info(
            "done /predict from=%s model=%s dur=%.3f preds=%d",
            client_ip,
            model or "default",
            duration,
            len(predictions),
        ...
        duration = time.perf_counter() - start
        logger.info(
            "err /predict from=%s model=%s dur=%.3f status=%s",
            client_ip,
            model or "default",
            duration,
        ...
        duration = time.perf_counter() - start
        logger.error(
            "err_unexpected /predict from=%s model=%s dur=%.3f",
            client_ip,
            model or "default",
            duration,
            exc_info=True,
...
async def predict_draw(request: Request, model: str | None = None):
    ...
    try:
        ...
        duration = time.perf_counter() - start
        logger.info(
            "done /predict_draw from=%s model=%s dur=%.3f",
            client_ip,
            model or "default",
            duration,
        )
        ...
        duration = time.perf_counter() - start
        logger.info(
            "err /predict_draw from=%s model=%s dur=%.3f status=%s",
            client_ip,
            model or "default",
            duration,
            getattr(e, "status_code", ""),
        ...
        duration = time.perf_counter() - start
        logger.error(
            "err_unexpected /predict_draw from=%s model=%s dur=%.3f",
            client_ip,
            model or "default",
            duration,
            exc_info=True,
