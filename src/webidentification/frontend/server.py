...
def _capture_page_slices(
    driver: webdriver.Remote, scroll_delay: float = 0.3
) -> list[tuple[int, bytes]]:
    device_pixel_ratio = int(
        driver.execute_script("return window.devicePixelRatio") or 1
    )

    total_height = driver.execute_script("return document.body.parentNode.scrollHeight")
    viewport_height = driver.execute_script("return window.innerHeight")
    if not isinstance(total_height, (int, float)) or not isinstance(
    ...
    while offset < total_height:
        ...
        if device_pixel_ratio > 1:
            ...
            img = img.resize(
                (img.width // device_pixel_ratio, img.height // device_pixel_ratio)
            )
            buf = BytesIO()
...
async def _forward_to_model(
    model_url: str, image_bytes: bytes, timeout_seconds: float
) -> bytes:
